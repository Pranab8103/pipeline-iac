#Resource-1 : create a Public IP Address for Azure Load Balancer
...
#Resource-2 : create an Azure load balancer 
...
#Resource-3 : Create backend Pool
resource "azurerm_lb_backend_address_pool" "web_backend_address_pool" {
    name = "web-backend"
...
resource "azurerm_lb_rule" "web_lb_rule_app1" {
    ...
    loadbalancer_id = azurerm_lb.web_lb.id
    backend_address_pool_ids = [azurerm_lb_backend_address_pool.web_backend_address_pool.id]
...
resource "azurerm_network_interface_backend_address_pool_association" "association" {
    ...
    #network_interface_id    = lement(azurerm_network_interface.windows_nic[count.index].id 
    ip_configuration_name   = azurerm_network_interface.windows_nic[each.key].ip_configuration[0].name
    backend_address_pool_id = azurerm_lb_backend_address_pool.web_backend_address_pool.id
}
